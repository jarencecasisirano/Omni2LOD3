...
def create_gml_polygon(coords):
    ...
    pos_list_str = " ".join([f"{x} {y} {z}" for x, y, z in coords])
...
def create_building_installation(min_pt, max_pt, cluster_id):
    ...
    desc.text = f"Slab generated from point cloud cluster {cluster_id}"
...
def main():
    # 1. Terminal File Selection
    las_path, gml_path = select_files()
    
    # 2. Read LAS and run DBSCAN
    print("Reading point cloud...")
    las = laspy.read(las_path)
    points = np.vstack((las.x, las.y, las.z)).transpose()
    
    print(f"Running DBSCAN on {len(points)} points... (This may take a moment)")
    clustering = DBSCAN(eps=2.0, min_samples=20).fit(points)
    labels = clustering.labels_
    unique_labels = set(labels)
    
    print(f"Found {len(unique_labels) - (1 if -1 in unique_labels else 0)} valid clusters.")
    ...
    # 4 & 5. Create slabs and append to GML
    appended_count = 0
    for label in unique_labels:
        ...
        installation_xml = create_building_installation(min_pt, max_pt, label)
    ...
    output_filename = gml_path.replace(".gml", "_trial2.gml")
    print(f"Saving modified GML to: {output_filename}")
    
    tree.write(output_filename, pretty_print=True, xml_declaration=True, encoding="utf-8")
    print(f"Success! Appended {appended_count} slabs.")
