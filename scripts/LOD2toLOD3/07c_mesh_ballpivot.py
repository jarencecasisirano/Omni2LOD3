...
def main():
    ...
    parser.add_argument("--radii",         type=str,   default=None,
                        help="Comma-separated ball radii in metres, e.g. 0.05,0.1,0.2 "
                             "(default: auto from point spacing)")
    parser.add_argument("--normal-radius", type=float, default=0.3,
                        help="Radius for normal estimation in metres (default 0.3)")
    ...
    if args.radii:
        ...
        v = input(
            "  Ball radii in metres, comma-separated [auto]: "
        ).strip()
        if v:
            manual_radii = [float(r.strip()) for r in v.split(",") if r.strip()]
    ...
    for las_path in selected_files:
        ...
        estimate_normals(pcd, radius=args.normal_radius, max_nn=args.normal_nn)

        radii = manual_radii if manual_radii else compute_auto_radii(pcd)
