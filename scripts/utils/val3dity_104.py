...
def _analyze_ring(ring, vertices, transform):
    ...
    if len(pts) < 3:
        return {"distinct_xy": len(set(pts)), "area": 0.0, "intersections": []}
    intersections = []
    n = len(pts)
    for i in range(n):
        i2 = (i + 1) % n
    ...
    return {"distinct_xy": len(set(pts)), "area": _polygon_area_xy(pts), "intersections": intersections}
...
def _expand_collapsed_ring(ring, vertices, transform, tol):
    ...
    if abs(analysis.get("area", 0.0)) >= area_tol:
        return ring, False
    out = list(ring)
    vidx = out[1]
    if not isinstance(vidx, int) or vidx < 0 or vidx >= len(vertices):
        return ring, False
    out[1] = _clone_vertex_with_xy_nudge(vertices, transform, vidx, delta_world=tol * 2.0, axis="y")
    post = _analyze_ring(out, vertices, transform)
    if abs(post.get("area", 0.0)) < area_tol:
        return ring, False
...
def _iter_104_targets(report_json):
    for feature in report_json.get("features", []):
...
def apply_104_fix_from_report(cityjson_data: dict, report_json: dict, tol: float = 0.001):
    ...
    for t in _iter_104_targets(report_json):
        ...
        if style == "flat":
            geoms = city_objects[coid].get("geometry", [])
            geom = geoms[t["geom"]]
            boundaries = geom.get("boundaries")
            gtype = geom.get("type", "")
            if gtype in ("Solid", "CompositeSolid"):
                boundaries[t["shell"]][t["face"]] = new_face
            else:
                boundaries[t["face"]] = new_face
        else:
            face_node[:] = new_face
        if touched:
