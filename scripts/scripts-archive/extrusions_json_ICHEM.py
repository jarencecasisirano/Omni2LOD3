...
def find_nearest_surface(points, vert_surfaces, dist_tol=5.0, z_expand=2.0):
    """
    Match a cluster to a wall surface via 3 stages.  Every stage
    enforces Z overlap so the matched wall is always at the right
    height relative to the cluster.

      1. Spatial intersection (Z overlap + signed distance + XY footprint).
      2. PCA normal alignment WITH Z overlap.
      3. Nearest centroid WITH Z overlap.
    """
    ...
    def _z_overlaps(vs):
        return z_hi >= vs["z_min"] - z_expand and z_lo <= vs["z_max"] + z_expand

    best, best_d = None, float("inf")

    # Stage 1 — bounded spatial intersection
    for vs in vert_surfaces:
        if not _z_overlaps(vs):
            continue
        n2d = vs["normal_2d"]
    ...
    # Stage 2 — PCA normal alignment WITH Z overlap
    ...
    # Stage 3 — nearest XY centroid WITH Z overlap
    best_d = float("inf")
    for vs in vert_surfaces:
        if not _z_overlaps(vs):
            continue
        d = float(np.linalg.norm(cxy - vs["origin_2d"]))
        if d < best_d:
            best_d, best = d, vs
...
# =====================================================================
def clip_bbox_at_wall(cluster_pts, wall_surface, vert_surfaces):
    """
    ...
    The near face is snapped to the nearest co-normal wall plane that
    lies at or behind the cluster's centroid, ensuring the extrusion
    is attached to the building surface.
    ...
    d_cen    = float(np.median(pt_d))  # cluster centre depth
    z_lo_cl  = float(pt_z.min())
    z_hi_cl  = float(pt_z.max())
    t_lo_cl  = float(pt_t.min())
    t_hi_cl  = float(pt_t.max())

    # Nothing protrudes outward — cluster is entirely inside the building
    if d_out <= d_wall:
        return None

    # ── Find the nearest wall plane to snap the near face ─────────────
    # Among all walls with similar normals that overlap the cluster in Z
    # and tangent, find the one whose plane depth (wall_d) is closest to
    # (but ≤) the cluster's centroid depth.  This attaches the extrusion
    # to the actual wall at the cluster's position.
    NORMAL_TOL = 0.90   # cosine similarity threshold
    best_snap_d = None

    for vs in vert_surfaces:
        # Must share the same facade orientation
        if abs(float(np.dot(vs["normal_2d"], n2d))) < NORMAL_TOL:
            continue
        # Must overlap vertically
        if vs["z_max"] < z_lo_cl - 1.0 or vs["z_min"] > z_hi_cl + 1.0:
            continue
        # Must overlap tangentially
        vs_t = vs["coords"][:, :2] @ txy
        if float(vs_t.max()) < t_lo_cl - 2.0 or float(vs_t.min()) > t_hi_cl + 2.0:
            continue

        # Wall plane depth projected onto our normal axis
        vs_d = float(np.dot(vs["origin_2d"], n2d))

        # Wall must be at or behind the cluster's centroid (not in front)
        if vs_d > d_cen:
            continue

        if best_snap_d is None or vs_d > best_snap_d:
            best_snap_d = vs_d

    # Fallback: use the matched wall's own wall_d
...
def main():
    ...
    for label in unique:
        ...
        clip = clip_bbox_at_wall(cluster_pts, matched, vert_surfaces)
