#!/usr/bin/env python3
"""
align_p2p_json_plus.py — Point-to-Point Reference Alignment
...
-----
    ...
    python scripts/LOD2toLOD3/04A_align_p2p_json_plus.py
...
class PointPicker3D:
    ...
    def __init__(self,
        ...
        # --- NEW: Nudge distance (in coordinate units, usually meters) ---
        self.nudge_step = 0.1 
    ...
    def _on_key(self, event):
        ...
        # --- NEW: Nudge Controls ---
            ...
            if not self.picked_pts:
                return  # Nothing to nudge yet
            
            # Change nudge step size
            if event.key == ']':
                self.nudge_step *= 2.0
                print(f"  Nudge step increased to {self.nudge_step:.3f}")
                return
            elif event.key == '[':
                self.nudge_step /= 2.0
                print(f"  Nudge step decreased to {self.nudge_step:.3f}")
                return

            # Apply nudge to the last picked point
            pt = self.picked_pts[-1].copy()
            if event.key == 'left':
                pt[0] -= self.nudge_step
            elif event.key == 'right':
                pt[0] += self.nudge_step
            elif event.key == 'down':
                pt[1] -= self.nudge_step
            elif event.key == 'up':
                pt[1] += self.nudge_step
            elif event.key in ('-', '_'):
                pt[2] -= self.nudge_step
            elif event.key in ('+', '='):
                pt[2] += self.nudge_step

            # Update visuals by replacing the last point
            self._undo()           # Remove the visual and list entry of the old position
            self._register_pick(pt) # Re-add it at the new nudged position
